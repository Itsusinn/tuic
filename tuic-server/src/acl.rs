...
pub(crate) fn parse_acl_rule(rule: &str) -> Result<AclRule, String> {
	...
	let hijack = if parts.len() > 3 { Some(parts[3].to_string()) } else { None };
...
pub(crate) fn parse_acl_address(addr: &str) -> Result<AclAddress, String> {
	...
	} else if addr.parse::<std::net::Ipv4Addr>().is_ok() || addr.parse::<std::net::Ipv6Addr>().is_ok() {
...
pub(crate) fn parse_acl_ports(ports_str: &str) -> Result<AclPorts, String> {
	let entries: Result<Vec<_>, _> = ports_str.split(',').map(|part| parse_acl_port_entry(part.trim())).collect();
