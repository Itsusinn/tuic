...
async fn curl_sniffer_integration() {
    ...
    // Configure to use local tuic server: assume tuic server already running on 127.0.0.1:1080 as SOCKS5
    // We'll call curl via --socks5-hostname to force hostname to be sent through proxy and let sniffer detect Host header
